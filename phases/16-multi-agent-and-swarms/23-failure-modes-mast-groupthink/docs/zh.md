# 故障模式 —— MAST、Groupthink、Monoculture 与级联错误

> 2026 年的参考分类体系是 **MAST**（Cemri 等人，NeurIPS 2025，arXiv:2503.13657），该体系基于对 7 个最先进的开源 MAS 的 1642 条执行轨迹分析得出，显示其**失败率为 41%–86.7%**。三大根本类别：**Specification Problems**（占 41.77%）——角色模糊、任务定义不清；**Coordination Failures**（占 36.94%）——通信中断、状态不同步；**Verification Gaps**（占 21.30%）——缺乏验证、缺失质量检查。**Groupthink** 家族（arXiv:2508.05687）补充了以下失效模式：monoculture collapse（相同基础模型 → 相关性失败）、conformity bias（智能体相互强化彼此的错误）、deficient theory of mind、mixed-motive dynamics、cascading reliability failures。级联示例：retry storms，即支付失败触发订单重试，进而触发库存重试，最终压垮库存服务（数秒内负载激增 10 倍——需要 circuit breakers）。Memory poisoning：一个智能体的 hallucination 进入共享内存，下游智能体将其视为事实；准确率逐渐衰减，导致根因诊断极为痛苦。**STRATUS**（NeurIPS 2025）报告称，通过部署专用的 detection / diagnosis / validation 智能体，可将缓解成功率提升 1.5 倍。本课程将故障模式视为一等公民的工程目标。
...
2026 年的生产实践是将故障模式作为设计输入。在你的架构中，除非你能针对每一个 MAST 类别明确指出已部署的缓解措施，否则它就不算“足够好”。
