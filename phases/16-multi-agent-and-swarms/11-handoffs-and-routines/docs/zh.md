...
> OpenAI 的 Swarm（2024年10月）将多 Agent 编排简化为两个基本原语：**Routines**（作为 system prompt 的指令 + tools）和 **Handoffs**（返回另一个 Agent 的 tool）。没有 state machine，也没有 branching DSL——LLM 通过调用正确的 handoff tool 来完成路由。OpenAI Agents SDK（2025年3月）是其 production 继任者。Swarm 本身仍然是最清晰的概念参考——其全部 source 仅几百行。该 pattern 之所以迅速流行，是因为其 API surface 大致为“agent = prompt + tools；handoff = function returning agent”。Limitation：stateless，因此 memory 是 caller 的问题。
