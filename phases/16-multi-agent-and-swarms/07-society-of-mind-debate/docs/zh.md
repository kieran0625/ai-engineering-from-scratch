...
> 明斯基在 1986 年提出的前提——智能是专家的社会——每隔十年就会被重新发现。2023 年，Du 等人将其转化为一种具体的算法：多个 LLM 实例提出答案，阅读彼此的答案，进行批评并更新。经过 N 轮迭代后，它们会收敛到一个共识，该共识在六项推理和事实性任务上优于零样本 CoT 和 Self-Reflection。有两个关键发现：**多个智能体**和**多轮交互**各自独立地贡献了效果提升。这种“社会”模式优于单智能体的独白；多轮交互优于一次性投票。
...
Self-consistency —— 多次采样同一个模型并取多数答案 —— 是你能够附加的最廉价的推理改进方法。它有效，但很快会遇到瓶颈。即使将采样数量翻倍，你也看不到再有实质性的提升。

Debate 打破了这种饱和状态。不再是来自单个模型的 N 个独立样本，而是 N 个智能体互相阅读彼此的推理过程并进行修正。样本间的相关性降低（它们不再 i.i.d.），而收敛点往往是正确的，这在 i.i.d. 投票必然得出错误结论时尤为明显。
