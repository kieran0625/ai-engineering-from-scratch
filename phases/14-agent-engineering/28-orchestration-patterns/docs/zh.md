# 编排模式：Supervisor-worker、Swarm、Hierarchical

> 在 2026 年的主流框架中，四种编排模式反复出现：supervisor-worker、swarm / peer-to-peer、hierarchical、debate。Anthropic 的建议是：“关键在于为你的需求构建合适的系统。”从简单开始；仅在单个智能体加上五种工作流模式仍不足以解决问题时，才引入拓扑结构。
...
- Implement all four in stdlib against a scripted LLM.
