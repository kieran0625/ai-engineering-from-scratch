...
> 你在第 04 课中从零构建了一个 GPT-2 Small。2026 年的前沿开源模型属于同一家族，仅包含五到六个具体的改动。使用 RMSNorm 替代 LayerNorm。使用 SwiGLU 替代 GELU。使用 RoPE 替代可学习的位置编码。使用 GQA 或 MLA 替代完整的 MHA。大规模使用混合专家（Mixture-of-Experts）。你已掌握的数学知识覆盖了其中 95% 的内容。本课将并排阅读 Llama 3、DeepSeek-V3、Mixtral、Qwen 和 Gemma 的架构，并明确指出每个架构发生分歧的具体代码行。
