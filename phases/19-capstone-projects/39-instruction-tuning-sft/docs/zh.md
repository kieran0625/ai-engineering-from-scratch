# 毕业设计课程 39：通过监督微调进行指令调优

> 预训练的基础模型可以扩展序列，但无法遵循指令。监督微调（SFT）是解决此问题的最小改动：向模型提供指令与期望响应的配对示例，并训练模型主体预测响应标记。关键在于，你只希望损失函数计算响应的部分，而不包括指令。本课程将构建一个 Alpaca 风格的 SFT 循环，使用自定义的 collate 函数将指令标记用 `ignore_index=-100` 进行掩码处理，在 200 个指令-响应对上训练，并在保留测试集上使用精确匹配进行评估。
