...
> 训练语言模型的工作远早于第一次前向传播就开始。语料库必须落地到磁盘，解压、去重并建立索引，而且在网络在 4% 进度时中断之前，断点续传的逻辑就已经要准备就绪。本课将构建一个流式下载器，用于拉取压缩分片（shards），使用 Zstandard 进行实时解压，通过 MinHash 结合局部敏感哈希（LSH）对近重复文档进行指纹识别，并写入下游管道可信赖的分片清单（manifest）。
