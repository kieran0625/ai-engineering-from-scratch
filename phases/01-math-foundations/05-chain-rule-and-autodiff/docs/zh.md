...
- 构建一个最小化的自动微分引擎（Value 类），能够记录操作并通过反向模式自动微分计算梯度
- 使用拓扑排序在计算图中实现前向传播与反向传播
- 仅使用从零构建的自动微分引擎，构建并训练一个用于 XOR 问题的多层感知机
- 使用梯度检查与数值有限差分对比，验证自动微分的正确性

## 问题描述

你可以计算简单函数的导数。但神经网络不是简单函数。它是数百个函数的组合：矩阵乘法、加上偏置、应用激活函数、再次矩阵乘法、softmax、交叉熵损失。输出是函数的函数的函数。

要训练网络，你需要损失相对于每一个权重的梯度。对于数百万参数，手工计算是不可能的。用数值方法（有限差分）又太慢。

链式法则给你数学原理。自动微分给你算法。两者结合，让你可以在与单次前向传播成正比的时间内，计算任意函数组合的确切梯度。
...
计算图让链式法则可视化。每个操作变成一个节点。数据沿图前向流动。梯度沿图反向流动。
...
神经网络有数百万输入（权重）和一个输出（损失）。反向模式在一次反向传播中计算所有梯度。这就是反向传播使用反向模式的原因。
...
这正是 PyTorch 的 `autograd` 所做的。`torch.Tensor` 类包装值，在 `requires_grad=True` 时记录操作，在调用 `.backward()` 时计算梯度。
...
5. 梯度通过加法累积在 `.grad` 属性中（而非替换）

图是动态的（运行时定义）。每次前向传播都会构建新图。这就是 PyTorch 支持模型内部控制流（if/else、循环）的原因。
...
基本的 Value 类处理加法、乘法和 relu。真正的自动微分引擎需要更多功能。以下是构建神经网络所需的操作：
...
巧妙之处：`__sub__` 和 `__truediv__` 用现有操作定义。它们自动获得正确的梯度，因为链式法则会通过底层的 add/mul/pow 操作进行组合。
...
`Neuron` 计算 `tanh(w1*x1 + w2*x2 + ... + b)`。`Layer` 是神经元的列表。`MLP` 堆叠各层。每个权重都是 `Value`，所以调用 `loss.backward()` 会将梯度传播到每个参数。
...
这就是 micrograd。一个用纯 Python 实现自动微分的完整神经网络训练循环。每个商业深度学习框架都在大规模上做同样的事情。

### 步骤 6：梯度检查

如何知道你的自动微分是正确的？与数值导数对比。这就是梯度检查。
...
梯度检查在实现新操作时至关重要。如果你的反向传播有 bug，数值检查能发现它。每个严肃深度学习实现都会在开发期间运行梯度检查。
...
梯度相同。你的引擎计算结果与 PyTorch 相同，因为数学原理相同：通过链式法则的反向模式自动微分。
...
1. 向 Value 类添加 `__pow__`，以便计算 `x ** n`。验证在 `x=2` 处 `d/dx(x^3)` 等于 `12.0`。

2. 添加 `tanh` 作为激活函数。验证 `tanh'(0) = 1` 和 `tanh'(2) = 0.0707`（近似）。

3. 为单个神经元构建计算图：`y = relu(w1*x1 + w2*x2 + b)`。计算全部五个梯度并与 PyTorch 验证。

4. 使用对偶数实现前向模式自动微分。创建 `Dual` 类并验证它给出的导数与反向模式引擎相同。

## 关键术语

| 术语 | 人们常说的 | 实际含义 |
|------|-----------|---------|
| 链式法则 | "把导数乘起来" | 复合函数的导数等于每个函数局部导数的乘积，在正确的点求值 |
| 计算图 | "网络结构图" | 有向无环图，节点是操作，边传递值（前向）或梯度（反向） |
| 前向模式 | "把导数往前推" | 从输入到输出传播导数的自动微分。每个输入变量需要一次传播。 |
| 反向模式 | "反向传播" | 从输出到输入传播梯度的自动微分。每个输出变量需要一次传播。 |
| 自动微分 | "自动梯度" | 记录值上的操作、构建图、并通过链式法则计算精确梯度的系统 |
| 对偶数 | "值加导数" | 形式为 a + b*ε（ε² = 0）的数，通过算术运算携带导数信息 |
| 拓扑排序 | "依赖顺序" | 对图节点排序，使每个节点都在其所有依赖之后。正确传播梯度所必需。 |
| 梯度累积 | "相加，不替换" | 当一个值输入到多个操作时，其梯度是所有传入梯度贡献的总和 |
| 动态图 | "运行时定义" | 每次前向传播都重新构建计算图，允许模型内部使用 Python 控制流（PyTorch 风格） |
| 梯度检查 | "数值验证" | 将自动微分梯度与数值有限差分梯度对比以验证正确性。对调试至关重要。 |
