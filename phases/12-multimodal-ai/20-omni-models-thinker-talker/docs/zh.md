# Omni Models: Qwen2.5-Omni 与 Thinker-Talker 架构拆分

> 2024年5月GPT-4o的产品演示之所以具有颠覆性，并非因为底层模型本身，而是因为它采用了全新的产品形态——一种语音交互界面：用户说话时，模型能看到摄像头捕捉的画面，并在250毫秒内做出语音回复。开源生态在2024年下半年至2025年全力追赶这一产品体验。Qwen2.5-Omni（2025年3月）是参考性的开源设计：由一个 Thinker（大型文本生成 Transformer）和一个 Talker（并行语音生成 Transformer）组成，两者通过流式语音 Token 进行连接。Mini-Omni 对其进行了简化，Moshi 达到了相同的延迟水平，GLM-4-Voice 则将其扩展至中文场景。本课程将深入解析 Thinker-Talker 架构，以及支撑流式实时对话的延迟预算分配。
