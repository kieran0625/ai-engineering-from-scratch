...
> 当模型无法准确判断何时该使用某个工具时，一个正确的工具也会静默失败。在 StableToolBench 和 MCPToolBench++ 等基准测试中，命名、描述和参数结构的设计会导致工具选择准确率出现 10 到 20 个百分点的波动。本课将阐述那些能将“模型可靠选中的工具”与“模型误触的工具”区分开来的设计规则。
...
- 使用“Use when X. Do not use for Y.”的模式编写工具描述，长度控制在 1024 characters 以内。
- 以稳定、`snake_case` 且在大 registry 中无歧义的方式命名工具。
- 针对特定 task surface，在 atomic tools 和单一 monolithic tool 之间做出选择。
