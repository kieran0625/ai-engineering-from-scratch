...
> OpenAI Preparedness Framework v2（2025年4月）引入了 Research Categories —— Long-range Autonomy、Sandbagging、Autonomous Replication and Adaptation、Undermining Safeguards —— 这些类别独立于 Tracked Categories。Tracked Categories 会触发 Capabilities Reports 与 Safeguards Reports，并由 Safety Advisory Group 进行审查。DeepMind 的 FSF v3（2025年9月发布，并于2026年4月17日增加 Tracked Capability Levels）将 autonomy 整合进 ML R&D 和 Cyber 领域（其中 ML R&D autonomy level 1 = 以具备竞争力的成本完全自动化 AI R&D pipeline，对比 human + AI tools）。FSF v3 明确通过针对 instrumental-reasoning misuse 的 automated monitoring 来应对 deceptive alignment。坦诚说明：PF v2 中的 Research Categories（包括 Long-range Autonomy）不会自动触发 mitigations；政策措辞为“potential”。DeepMind 自身也指出，如果 instrumental reasoning 持续增强，automated monitoring “will not remain sufficient long-term”。
