...
> 将前沿的编码模型与进化循环和 machine-checkable evaluator 配对。让循环运行足够长的时间。它会发现一种使用 48 次标量乘法的 4x4 复数矩阵乘法过程——这是 56 年来对 Strassen 算法的首次改进。它还会找到一个覆盖全公司的 Borg 调度启发式算法，能在生产环境中回收约 0.7% 的集群算力。该架构故意设计得平淡无奇。真正的优势在于 evaluator 的严谨性。
